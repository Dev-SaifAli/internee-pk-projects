/**
 * Qubi AI Assistant - Core Logic
 * An enterprise-grade chat interface controller handling state, 
 * storage, and simulated bot interactions.
 */
    ...
    addMessageToUI(msg) {
        ...
        const template = `
            <div class="flex items-start gap-4 message-slide-in ${isBot ? '' : 'flex-row-reverse'}">
                ...
                <div class="flex flex-col ${isBot ? 'items-start' : 'items-end'} max-w-[75%]">
                    <div class="py-3 px-4 rounded-2xl ${isBot ? 'bg-white border border-gray-100' : 'bg-purple-100 text-gray-900 border border-purple-200 shadow-sm'}">
                        <p class="text-[15px] leading-relaxed break-words">${msg.text}</p>
        ...
        this.nodes.chatMessages.insertBefore(wrapper.firstElementChild, this.nodes.typingIndicator);
        this.scrollToBottom();
    }
